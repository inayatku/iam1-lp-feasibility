"""Cross-check every numeric table in the manuscript against a results directory.

Iteration counts are pivots performed, counted uniformly for all methods; an
instance feasible at the initial basis counts as zero. Pan's averages are over
its successful runs only (IAM-1, simplex, and HiGHS solve every random
instance), so for the others Avg_Iterations == Avg_Iterations_All. Wall-clock
timings are machine-dependent and are reported, not asserted. NETLIB values are
read from netlib_batch_maxiter6000_results.csv over the 52-problem set (54
distinct minus QAP15 and MAROS-R7).
...
for sp in (0.0, 0.5):
    for ptype, col in (("feasible", 0), ("infeasible", 1)):
        ...
        for size in SIZES:
            ...
            check(f"{size} {ptype} sp{sp} Pan iters", exp[2+col], float(r[f"{PAN}_Avg_Iterations"]))
...
for sp in (0.0, 0.5):
    ...
    for (label, fcol), expect in zip(
        ...
        m = (float(fr["60x60"][fcol]) + float(ir["60x60"][fcol])) / 2
...
if os.path.exists(NB):
    ...
    check("NETLIB set size", 51, len(probs), tol=0)

    LIM = 6000

    def nval(name, col):
        return float(last[name][col])
    ...
    check("NETLIB common-set size", 41, len(common), tol=0)
    # Table 9 (tab:netlib_overall)
    T9 = {"iam1_iterations": (1, 26.08, 13.61), "pan_row_scaled_iterations": (7, 15.55, 13.44),
          "simplex_iterations": (6, 35.85, 28.87), "highs_simplex_iterations": (0, 31.47, 15.26)}
