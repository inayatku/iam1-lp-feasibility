...
NETLIB result log. Performance measure: iteration counts; runs that hit the
...
LOG = os.path.join(HERE, "..", "results", "netlib_batch_maxiter6000_results.csv")
...
# performance data; runs that hit the iteration limit -> inf
def value(p, col):
    return float(last[p][col])
